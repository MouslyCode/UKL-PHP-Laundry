<?php 
    ...
    if($dt_histori['status_order']=='baru'){
        $update=mysqli_query($conn,"update transaksi set status_order = 'proses' where id = '".$_GET['id']."'") or die(mysqli_error($conn));
    } elseif($dt_histori['status_order']=='proses'){
        $update=mysqli_query($conn,"update transaksi set status_order = 'selesai' where id = '".$_GET['id']."'") or die(mysqli_error($conn));
    } elseif($dt_histori['status_order']=='selesai'){
        $update=mysqli_query($conn,"update transaksi set status_order = 'diambil' where id = '".$_GET['id']."'") or die(mysqli_error($conn));
    }
    if($update){
